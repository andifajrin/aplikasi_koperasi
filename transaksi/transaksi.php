...
<?php
		...
		$qubah=mysqli_query("SELECT * FROM t_anggota WHERE kode_anggota='$kode'");
...
{ $kode=$_GET['kode_anggota']; $anggota=mysqli_fetch_array(mysqli_query($koneksi,"SELECT nama_anggota from t_anggota where kode_anggota='$kode'")); ?>
  ...
  <div class="col-lg-12">
     <div class="form-panel">
        ...
        <?php $df=mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc"));$op=mysql_num_rows($df);
...
{ 
 ...
 {
 	$baru=mysqli_fetch_array(mysqli_query($koneksi,"SELECT *FROM t_simpan where kode_anggota='$kode' and jenis_simpan='wajib' order by kode_simpan desc "));$numrow=mysql_num_rows($baru);
 	$data=$baru['tgl_mulai'];
 	$now=date("Y-m-d");
 	if($data==$now)
 	{
